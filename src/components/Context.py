...
class Context:
    """The context class is used to store the context of the environment.
    """
    def __init__(self, origin_adr, destination_adr):
        ...
        self.lngg = 0
        self.make_map()
        self.battery = Battery(50000)  # Wh
        self.energy_calculator = Energy()
        self.charge_num = 0
    ...
    def geocoding_api(self, address):  # 2 output: status, position
        ...
        return (
            self.geocode_json_status, 
            self.geoposition, 
            self.geoposition_tuple
        )
    ...
    def elevation_api(self, location):  # 2 output: elevation, resolution
        ...
        elevation_url = elevation_api + \
            urllib.parse.urlencode({'locations': location}) + \
            "&key=AZuesYuds12_dsakd23456sdeHf"
    ...
    def directions_api(self, origin, destination):  # 5 output: status, steps, bound
        ...
        if self.directions_json_status == 'OK':
            ...
            # all are 'southwest', 'northeast'
            directions_data_routes_bounds = directions_data_routes['bounds']
            ...
            self.bound = {'north': self.north, 'east': self.east,
                          'south': self.south, 'west': self.west}  # value of lat/lng
        else:
    ...
    def make_map(self):
        origin_status, origin_position, origin_position_num = self.geocoding_api(
            self.origin)
        if origin_position_num == ('g', 'g'):
            origin_position_num = (self.latt, self.lngg)
        destination_status, destination_position, destination_position_num = self.geocoding_api(
            self.destination)
        if destination_position_num == ('g', 'g'):
            destination_position_num = (self.latt, self.lngg)
        direction_status, direction_step, self.map_bound = self.directions_api(
        ...
        self.end_position = destination_position_num
    ...
    def step(self, action):  # output:
        ...
        if status != 'OK' or self.next_position[0] > self.map_bound['north'] or self.next_position[0] < self.map_bound['south'] or self.next_position[1] > self.map_bound['east'] or self.next_position[1] < self.map_bound['west']:
            ...
            for i in range(len(leg_step)):
                ...
                speed = math.sqrt(distance ** 2 + elevation **
                                  2) / duration  # m/s
                angle = math.atan2(distance * 1000, elevation)   # degree
                angle = angle if angle > 0 else 0
                power = self.energy_calculator.run(road_angle=angle, driving_speed=speed)
                energy_consume = 0
            ...
            if abs(self.next_position[0] - self.end_position[0]) < self.stride_height and abs(self.next_position[1] - self.end_position[1]) < self.stride_wide:
                ...
                statusE, leg_stepE, boundE = self.directions_api(
                    next_position, end_position)  # fix
                self.legE = leg_stepE
                if statusE == 'OK':
                    for i in range(len(self.legE)):
                        ...
                        while status != 'OK' or status1 != 'OK':                   # recheck again
                            status, height_start = self.elevation_api(
                                start_position)
                            status1, height_end = self.elevation_api(
                                end_position)
                        elevation = height_end - height_start  # unit: m
                        if duration <= 0:
                            duration = 1
                        ...
                        angle = math.atan2(
                            distance * 1000, elevation)   # degree
    ...
    # to get the step_reward, chargenum, SOC, time which the route google provided
    def origine_map_reward(self):
        leg = self.Google_step
        step_reward = 0
        time = 0
        for i in range(len(leg)):
            start = (leg[i]['start_location']['lat'],
                     leg[i]['start_location']['lng'])
            end = (leg[i]['end_location']['lat'],
                   leg[i]['end_location']['lng'])
