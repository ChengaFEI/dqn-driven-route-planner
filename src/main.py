from components.Context import Context as environment
from networks.DQN import DQN as Qnetwork
import numpy as np
...
class experience_replay_buffer():
    ...
    def append(self, exp):
        ...
        if len(self.buffersave) + len(exp) >= self.buffersize:
            self.buffersave[0:len(self.buffersave) +
                            len(exp) - self.buffersize] = []
        if len(self.currents1) + len(exp) >= self.buffersize:
            self.currents1[0:len(self.currents1) +
                           len(exp) - self.buffersize] = []
        if len(self.currents2) + len(exp) >= self.buffersize:
            self.currents2[0:len(self.currents2) +
                           len(exp) - self.buffersize] = []
...
map_wide = haversine(upper_left, upper_right) if haversine(upper_left, upper_right) > haversine(
...
if load_model == True:
    ...
    for current1, current2, action, reward, next1, next2, ds in zip(buffercurrents1, buffercurrents2, bufferactions, bufferrewards, buffernexts1, buffernexts2, bufferds):
        ...
        if ds == 'False':
            boo = False
        replay_buffer.buffer.extend(np.reshape(np.array([[float(current1), float(current2)], int(
...
parameter = [env.map_bound, map_height, map_wide, step_length, wide_grid_num, height_grid_num,
             total_point, max_train_step, pre_train_step, learning_rate, step_rewardG, chargenumG, SOCG, timeG]
df_1 = pd.DataFrame([parameter])
df_1.to_csv("./ev/train_para.csv", header=["google map_boundary", "map_height(km)", "map_wide(km)", "step_length(m)", "wide_grid_num", "height_grid_num",
            "total_point", "max_train_step in episode", "pre_train_step", "learning_rate", "Google r", "Google charge num", "Google SOC", "Google time"])
    ...
    for episode in range(train_num):  # num of episode
        ...
        while (in_ep_step <= max_step):   # Max step in one episode
            ...
            if np.random.rand(1) < e or (total_step < pre_train and load_model == False):
                ...
                # inputt = sess.run(Qnet.inputt, feed_dict={Qnet.input:[s_list]})[0]
                action = sess.run(Qnet.predict, feed_dict={
                                  Qnet.input: [s_list]})[0]
            ...
            if (total_step > pre_train and env.status_dir_check != 'OVER_QUERY_LIMIT' and len(replay_buffer.buffer) > batch_num) or (load_model == True and len(replay_buffer.buffer) > batch_num):  # start updating model
                ...
                y = ex_batch[:, 2] + mul * gamma * \
                    Targetnet_action[range(batch_num), Qnet_pre]  # target Q
                # print(y)
                ...
                loss = sess.run(Qnet.loss, feed_dict={Qnet.input: np.vstack(
                    # We use Qnet loss to update Qnet
                ...
                # print(loss)
                # update model
                _ = sess.run(Qnet.update, feed_dict={Qnet.input: np.vstack(
                    # input is s not s1
                    ex_batch[:, 0]), Qnet.target_y: y, Qnet.a: ex_batch[:, 1]})
                istrain = 1  # try
            ...
            if d == True:
                ...
                history = [episode+1, in_ep_step, time, episode_reward, real_reward, real_r_nofail, charge_num, SOC,
                           env.unreach_position_num, d, random_a/(random_a+network_a), avg_loss, overQ_num, loss_history, step_buffer]
                if episode == 0:
                ...
                if in_ep_step < total_point/2:
                    ...
                    print("realp_reward, chargenum, SOC, time: ",
                          step_rewardG, chargenumG, SOCG, timeG)
                    print("Our route step less than" +
                          str(total_point/2) + "  >>>>>>>>>>>>>>>>>>>")
